...
void sub()
{
	...
	while(!q.em)
	{
		auto [d,y,x]=q.top();
		q.pop();
		if(dis[y][x]>d)//現在の最短距離よりも辺の方が短いなら更新
		{
			ans+=v[y][x];
			//連結成分の要素の最短距離を更新
			dis[x][y]=dis[y][x]=v[y][x];
			for(ll i=0;i<n;++i)
			{
				for(ll j=0;j<n;++j)
				{
					chmin(dis[i][j],dis[i][x]+dis[x][j]);
				}
			}
			for(ll i=0;i<n;++i)
			{
				for(ll j=0;j<n;++j)
				{
					chmin(dis[i][j],dis[i][y]+dis[y][j]);
				}
			}
		}
		if(dis[y][x]<d)//現在の最短距離よりも辺の方が長いなら成立しない
		{
			ff(-1);re;
		}
	}
