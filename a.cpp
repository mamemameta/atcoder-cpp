...
void sub() {
	...
	mod=998244353;
	vec<vec<mint>>dp(n,vec<mint>(5));
	/*
	 *dp[頂点i][頂点iから任意のパスの危険交差点の最大数]=組合せ数
	 */
	function<void(ll,ll)>dfs=[&](ll now,ll pre)
	{
		//子に伝搬
		for(ll to: e[now])if(to!=pre)
		{
			dfs(to,now);
		}
		//現在の頂点を使うかどうか場合
		for(ll t=0;t<=1;++t)
		{
			...
			{
				array<mint,3>tmp={};
				for(ll i=0;i<3;++i)//i=木のpathの最大値
				{
					for(ll j=0;j+t<3&&i+j<3;++j)//j=新しい子の最大値
					{
						tmp[max(i,j+t)]+=g[i]*dp[to][j];
					}
				}
				g=tmp;
			}
