...
struct SegTreeLazy {
	...
	vector<X> dat;
	vector<A> lazy;
	SegTreeLazy(int n_):dat(n_*4,ex),lazy(n_*4,ea){
			int x = 1;
			while (n_ > x) x *= 2;
			n = x;
			//fl("n",n);
...
void sub(){
	...
	struct train {
		...
		// 存在しなければ nullopt を返す
		ll  get_last_train(ll  time_limit) const {
	...
	pq<pll> pq;//時刻，駅
	vll ans(N);//ans[i]:=駅iから出発した時,駅n-1に行くための一番遅い出発時刻
	ans.ba=MAX;
	// 駅 N に十分遅い時間にいるところから探索を始める
	pq.push({MAX,N-1});
	while (!pq.empty()) {
		// まだ探索していない駅のうちもっとも遅くまでいられる駅を探索する
		auto[time_limit, now]=pq.top();
		pq.pop();
		//time_limitが駅nowから出発してn-1に着くまでの一番遅い時刻ans[now]より早いならcontinue
		if (time_limit < ans[now] )
		{
			continue;
		}
		// 駅 now に向かう電車情報に対して
		for (auto e : train_info[now]) {
			ll next=e.from;
			ll next_time=e.get_last_train(time_limit);
			// 駅 next の最終時刻 ≥ 駅 now の最終時刻に間に合う電車の発射時刻
			if (next_time && ans[next] < next_time)// 更新できるなら
			{
				ans[next]=next_time;
				pq.emplace(next_time, next); // 優先度つきキューに入れる
			} 
		}
	}
	i(N-1)
