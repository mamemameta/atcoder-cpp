...
void sub() {
	ge(st,S,T);
	const ll N=S.sz,M=T.sz;
	S="0"+S;
	T="0"+T;
	vec<vll>memo(N+1,vll(M+1,-1));
	fo(n,0,N)
		memo[n][0]=0;
	fo(m,0,M)
		memo[0][m]=0;
	function<ll(ll,ll)>f=[&](ll n,ll m)
	{
		if(memo[n][m]!=-1)
			return memo[n][m];
		return memo[n][m]=max({f(n-1,m-1)+(S[n]==T[m]),f(n,m-1),f(n-1,m)});
	};
	f(N,M);
	function<st(ll,ll)>g=[&](ll n,ll m)
	{
		if(n==0||m==0)
			return string();
		if(f(n-1,m)==f(n,m))
		{
			return g(n-1,m);
		}else if(f(n,m-1)==f(n,m))
		{
			return g(n,m-1);
		}else
		{
			return g(n-1,m-1)+S[n];
			//return g(n-1,m-1);
		}
	};
	cout<<g(N,M)<<endl;
