...
void sub() {
	...
	function<dou(ar)>f=[&](ar cnt)
	{
		if(vis[cnt[0]][cnt[1]][cnt[2]])
			return memo[cnt[0]][cnt[1]][cnt[2]];
		vis[cnt[0]][cnt[1]][cnt[2]]=1;
		dou sum=N;
		for(ll c=1;c<=3;++c)
		{
			if(cnt[c])
		...
		sum/=((dou)(N-cnt[0]));
		return memo[cnt[0]][cnt[1]][cnt[2]]=sum;
	};
	fix(10,f(cnt));
	/*
	for(auto [v,c]:memo)
	{
		cc(v[0],v[1],v[2],v[3],c);
		ff();
	}
	*/
}
