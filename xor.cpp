...
int main(){
   ...
   if(mp.sz==1){
		 cj((mp.be)->fs==0?"Yes":"No");
		 rz;

   }
   if(mp.sz==2){
		 ll a=(mp.be)->fs,b=(mp.be)->se;
		 ll aa=next(mp.be)->fs,bb=next(mp.be)->se;
		 //cj("a",a,"b",b,"aa",aa,"bb",bb);
		 cj(a==0&&2*b==bb&&(a^aa==aa)?"Yes":"No");
		 rz;
   }
   if(mp.sz==3){
		 auto it=mp.be;
