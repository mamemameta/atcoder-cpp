...
void sub() {
	...
	while(ok+1<ng)
	{
		...
		i(t.sz)
		{
			...
			//f文字目以降で文字がcである添字の番目
			ll it=lower_bound(mp[c].be,mp[c].en,f)-mp[c].be;
			/*
			cc("v");ou(mp[c]);
			ff("l",l);
			*/
			...
			need-=k*l;
			f=mp[c].ba+1;
			//ff("k",k,"used",used,"need",need,"f",f);
			if(need>0)
			{
				used++;
				f=mp[c][need-1]+1;
		...
		{
			ok=m;
			//ff("ok",ok,"used",used,"f",f-1);
		}else
		{
			ng=m;
			//ff("ng",ng,"used",used,"f",f-1);
		}
	}
	ff(ok);
}
