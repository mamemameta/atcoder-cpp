...
void sub() {
	...
	ll n=20;
	cnt[1]=7*7*7;
	for(ll ax=0;ax+6<=13;++ax)
	{
	for(ll ay=0;ay+6<=13;++ay)
	{
	for(ll az=0;az+6<=13;++az)
	{
		for(ll x=0;x<=6;++x)
		{
		for(ll y=0;y<=6;++y)
		...
		for(ll z=0;z<=6;++z)
		{
			cnt[v[ax+x][ay+y][az+z]]--;
			v[ax+x][ay+y][az+z]++;
			cnt[v[ax+x][ay+y][az+z]]++;
		}
		}
		}
	for(ll bx=0;bx+7<n;++bx)
	{
	for(ll by=0;by+7<n;++by)
	{
	for(ll bz=0;bz+7<n;++bz)
	{
		for(ll x=0;x<=6;++x)
		{
		for(ll y=0;y<=6;++y)
		...
		if(cnt[1]==c1&&cnt[2]==c2&&cnt[3]==c3)
		{
			ff("Yes");
			ff(7,7,7,ax,ay,az,bx,by,bz);
			re;
		}
		for(ll x=0;x<=6;++x)
		{
