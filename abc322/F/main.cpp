...
struct SegTreeLazy {
    ...
    vector<X> dat;
    vector<M> lazy;
    SegTreeLazy(int n_, FX fx_, FA fb_, FM fm_, X ex_, M emm_)
...
struct S{
	ll x,l,mx;
	S(ll _x=0,ll _l=0,ll _mx=0):x(_x),l(_l),mx(_mx)
	{
...
void sub()
{
	...
	M emm=0ll;
	int N=5e5+5;
	SegTreeLazy sg(N,fx,fb,fm,ex,emm);
	ge(st,s);
	i(n)
	{
		if(s[i]=='1')
