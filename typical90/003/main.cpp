...
void sub() {
	ll ans=0;
	vll cnt={5,4,3,2};
	function<void(ll,ll,vll)>dfs=[&](ll hito,ll s,vll v)
	{
		if(hito==4)
		{
			ans+=s>acc(v);
		...
		for(ll ns=0;ns<=s;++ns)
		{
			for(ll c0=0;c0<=v[0];++c0)if(ns>c0)
			{
			for(ll c1=0;c1<=v[1];++c1)if(ns>c0+c1)
			{
			for(ll c2=0;c2<=v[2];++c2)if(ns>c0+c1+c2)
			{
			for(ll c3=0;c3<=v[3];++c3)if(ns>c0+c1+c2+c3)
			{
					vll t=v;
					t[0]-=c0;
					t[1]-=c1;
