...
struct Rerooting {
    ...
    struct DP {  // DP の型
        long long dp;
	ll order=1;
        DP(long long dp_,ll _o) : dp(dp_),order(_o) {}
        DP(long long dp_) : dp(dp_),order(1) {}
	DP operator+(DP r) {
		DP ret=DP(dp,order);
		ret.dp=dp      +r.dp;
		ret.order=order+r.order;
		return ret;
	}
    };
    const DP identity = DP(0,0);  // 単位元(末端の値は add_root(identity) になるので注意)
    function<DP(DP, DP)> merge = [](DP dp_cum, DP d) -> DP {
        return DP(dp_cum.dp+ d.dp ,dp_cum.order+d.order);
    };
    function<DP(DP)> add_root = [](DP d) -> DP {
        return DP(d.dp + d.order,d.order+1);
    };
    /* end 問題ごとに書き換え */
...
void sub() {
	...
	vll ans(n,-1);
	function<void(ll,ll)>f=[&](ll now,ll pre)
	{
		for(auto [to,w]:e[now])if(ans[to]==-1){
			ans[to]=ans[now]^w;
			f(to,now);
		}
	};
