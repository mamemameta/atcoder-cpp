...
void sub() {
	...
	while(!q.empty())
	{
		...
		if(vis.count(x))
			continue;
		ll sum=0,turn=0,tmp=0;
		vll v;
		for(ll i=x;i<=1e6&&mp.count(i);i+=d)
		{
			vis.in(i);
			v.pub(mp[i]);
		}
		//ou(v);
		vec<vll> dp=vec(2,vll(v.sz+5,1e8));
		dp[0][0]=v[0];
		dp[1][0]=0;
		for(ll i=0;i<=v.sz;++i)
		{
			chmin(dp[0][i+1],min(dp[0][i],dp[1][i])+v[i+1]);
			chmin(dp[1][i+1],dp[0][i]);
		}
		//ouu(dp);
		ans+=min(dp[0][v.sz-1],dp[1][v.sz-1]);
	}
	ff(ans);
}
