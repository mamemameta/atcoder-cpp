...
void sub() {
	...
	ge(ll,N,K);
	ge(st,S);
	vec<map<st,mint>>memo(N+1);
	function<ll(st)>same=[&](st t)
	{
		ll ok=1;
		for(ll l=0,r=t.sz-1;l<r;++l,--r)
		{
			ok&=t[l]==t[r];
		}
		return ok;
	};
	function<mint(ll,st)>f=[&](ll n,st s)
	{
		//ff("n",n,s);
		if(n==N)
		{
		...
		if(S[n]!='?'){
			if(s.sz==K-1)
			{
				...
				{
					//ff("here");
					return memo[n][s]=mint(0);
				}
				else
				{
					st ss=s.substr(1);
					return memo[n][s]=f(n+1,ss+S[n]);
				}
			}else 
				return memo[n][s]=f(n+1,s+S[n]);
		}else
		{
			mint m=0;
			for(ch c='A';c<='B';++c)
