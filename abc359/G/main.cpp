...
struct Rerooting {
	...
	*/
    ...
    function<DP(DP, DP)> merge = [](DP dp_cum, DP d) -> DP {
	    for(ll i=0;i<d.sum.sz;++i)
		   {
			   d.sum[i]+=dp_cum.sum[i];
			   d.cnt[i]+=dp_cum.cnt[i];
		   }
	    return d;
    };
    struct Edge {
    ...
    function<DP(DP,DP)> add_root = [](DP d,DP v) -> DP {
	    i(d.sum.sz)
		   {
			   d.sum[i]+=v.sum[i]+v.cnt[i];
			   d.cnt[i]+=v.cnt[i];
		   }
	    return d;
    };
    ...
    void bfs(int v, const DP& dp_p, int p = -1) {  // bfs だが、実装が楽なので中身は dfs になっている
        ...
        ans[v] = add_root(dp_l[deg],dp_l[deg]);  // 頂点 v の答え

        for (int i = 0; i < deg; i++) {  // 一つ隣の頂点に対しても同様に計算
            int u = G[v][i].to;
            if (u == p) continue;
            //bfs(u, add_root(merge(dp_l[i], dp_r[i + 1]),v), v);
            bfs(u, add_root(merge(dp_l[i], dp_r[i + 1]),dp_l[i]), v);
        }
    }
};
void sub(){
	...
	ll ans=0;
	vll used(N),siz(N);
	auto slv=[&](auto slv,ll v)->void
	{	
		auto findCd=[&](ll v)->ll//vを含む連結成分の重心を返す
		{
			//vを含む連結成分について
			//vを根とした部分木の位数を計算
			auto cnt=[&](auto cnt,ll v,ll p)->ll
			{
				siz[v]=1;
				for(ll to:e[v])if(to!=p&&used[to]==0)
				{
					siz[v]+=cnt(cnt,to,v);
				}
				return siz[v];
			};
			ll total=cnt(cnt,v,-1);
			ll c=0;
			//最大の部分木がtotal/2以下である頂点を探す
			auto find=[&](auto find,ll v,ll p)->void
			{
				ll ok=(total-siz[v])*2<=total;
				for(ll to:e[v])if(to!=p&&used[to]==0)
				{
					find(find,to,v);
					ok&=siz[to]*2<=total;
		...
		mll cnt,sum;
		cnt[a[c]]++;
		for(ll to:e[c])if(used[to]==0)
		{
			vec<pll>ps;
			auto path=[&](auto path,ll v,ll p,ll dep=1)->void
			{
				ps.pub({a[v],dep});
				for(ll to:e[v])if(to!=p&&used[to]==0)
				{
					path(path,to,v,dep+1);
				}
			};
			path(path,to,-1);
			for(auto [col,d]:ps)
			{
				ans+=cnt[col]*d+sum[col];
			}
			for(auto [col,d]:ps)
			{
				cnt[col]++;
