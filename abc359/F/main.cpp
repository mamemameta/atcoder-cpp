...
struct segtree {
	struct X{
		ll x=0;//長さ
		ll pos;
		X(ll _x,ll _p):x(_x),pos(_p){;}
		X(ll _x):x(_x){;}
		X():x(0){;}
		bool operator==(X r) {re r.x==x;}
		bool operator<(X r) {re x<r.x;}
	};
	X opx(X x1,X x2)
	{
		if(x1.x<x2.x)
...
void sub() {
	...
	in(a);
	so(a);
	segtree use(N+5),rest(N+5);
	
	use.set(0,segtree::X(a[0],0));
	for(ll i=1;i<N;++i)
	{
		rest.set(i,segtree::X(a[i],i));
	}
	use.build();
	rest.build();
	ll ans=0;
	i(N-1)
	{
		segtree::X u=use.query(0,N);
		segtree::X r=rest.query(0,N);
		ans+=u.x+r.x;
		ll iu=u.pos;
		ll ir=r.pos;
		//ff(iu,u.x,ir,r.x);
		use.update(iu,segtree::X((u.x/a[iu]+2)*a[iu],iu));
		use.update(ir,segtree::X(3*a[ir],ir));
		rest.update(ir,segtree::X(MAX,ir));
	}
	ff(ans);
