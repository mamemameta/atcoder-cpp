...
void sub() {
	...
	for (int i = 0; i < n - 1; i++) {
		...
		sg.update(t.in[v[i]],w[i]);
		sg.update(t.out[v[i]],-w[i]);
	}
	ge(ll,q);
	while (q--) {
		fl("q",q);
		...
		if (o == 1) {
			x--;
			sg.update(t.in[v[x]],y-w[x]);
			sg.update(t.out[v[x]],w[x]-y);
			w[x] = y;
		} else {
			x--, y--;
			int z = t.lca(x, y);
ll ans = sg.ran(0,t.in[x]) + sg.ran(0,t.in[y] ) - 2 * sg.ran(0,t.in[z]);
			fl(ans);
		}
	}
}
