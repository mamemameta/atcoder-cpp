...
void sub()
{
	...
	vec<st>v(h);
	in(v);
	vec<vll> row(h,vll(30)),col(w,vll(30));
	rep(r, h) rep(c, w) {
		row[r][v[r][c] - 'a']++;
		col[c][v[r][c] - 'a']++;
	}
	int hc = h, wc = w;
	vector<bool> frr(h), fc(w);
	rep(_, h + w) {
		vec<pll> ur, uc;
		rep(r, h)if (!frr[r]) {
			rep(j, 26) {
		...
		for (pair<int, int> p : ur) {
			frr[p.first] = true;
			rep(i, w) col[i][p.second]--;
			hc--;
		}
		for (pair<int, int> p : uc) {
			fc[p.first] = true;
	...
	for(ll r=0;r<h;++r)
	{
		for(ll ca=0;ca<26;++ca)
		{
			ff("r",r,"ca",ca,"cnt",row[r][ca]);
		}
	}
	ff("col");
	for(ll c=0;c<w;++c)
