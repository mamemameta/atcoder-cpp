...
struct SegTreeLazy {
	...
	vector<X> dat;
	vector<Y> lazy;
	SegTreeLazy(int n_, FX fx_, FA fb_, FY fm_, X ex_, Y emm_)
		: n(), fx(fx_), fb(fb_), fm(fm_), ex(ex_), emm(emm_), dat(n_ * 4, ex), lazy(n_ * 4, emm) {
			int x = 1;
			while (n_ > x) x *= 2;
			n = x;
		}
	...
	/* lazy eval */
	void eval(int k) {
		if (lazy[k] == emm) return;  // 更新するものが無ければ終了
		if (k < n - 1) {            // 葉でなければ子に伝搬
			lazy[k * 2 + 1] = fm(lazy[k * 2 + 1], lazy[k]);
			lazy[k * 2 + 2] = fm(lazy[k * 2 + 2], lazy[k]);
		}
		// 自身を更新
		dat[k] = fb(dat[k], lazy[k]);
		lazy[k] = emm;
	}

	void update(int a, int b, Y x, int k, int l, int r) {
		eval(k);
		if (a <= l && r <= b) {  // 完全に内側の時
			lazy[k] = fm(lazy[k], x);
			eval(k);
	...
	void update(int a, int b, Y x) { update(a, b, x, 0, 0, n); }
...
Struct S{
	mint d,sum;
	S(mint _d=1 ,mint _sum=0):d(_d),sum(_sum)
	{
		;
	}
};
void sub()
{
	...
	using X=S;
	using Y=mint;
	function<X(X,X)> fx=[](X x1,X x2)
	{

	}
	SegTreeLazy sg(n+5,fx,fb,fm,ex,emm);
}
