...
void sub()
{
	...
	vec memo(n+5,vec<ll>(10,-1));
	memo[n-1][0]=1;
	for(ll i=1;i<10;++i)
		memo[n-1][i]=0;

	function<ll(ll,mint)>dfs=[&](ll i,mint in)//深さ,値
	{
		ll b=0;
		ll d=x[i]-'0';
		if(i==n-1)
		{
			if(s[i]=='A')//mod 7にしたくない
			{
				b=memo[i][in.n]&&memo[i][(in+d).n];
			}else //mod 7にしたい
			{
				b=memo[i][in.n]||memo[i][(in+d).n];
			}
		...
		if(s[i]=='A')//mod 7にしたくない
		{
			b=dfs(i+1,in*10)&&dfs(i+1,(in+d)*10);
		}else //mod 7にしたい
		{
			b=dfs(i+1,in*10)||dfs(i+1,(in+d)*10);
		}
	...
	ff(dfs(0,mint(0)) ? "Takahashi" : "Aoki");
}
