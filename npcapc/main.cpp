...
void sub() {
	...
	init_f();
	vll cnt(N+1);
	vec<lazy>sg(N+1,lazy(M+1));
	vec<vll>memo(N+1,vll(M+1));
	function<vll(ll,ll)>f=[&](ll n,ll sum)
	{
		//fl("n",n,sum);
		if(n==N+1)
			return {};
		if(vis[n][sum])
			return {};
		if(sg[n].query(sum,sum+1).x==0)
			return {};
		if(sum+a[n]*(N-n+1)<M)
		{
			sg[n].update(0,sum+1,0);
			return {};
		}
		vll v(N+1);
		if(sum+a[n]>=M)
		{
			ll r=N-n;
			//ff("n",n,"sum",sum+a[n],"r",r);
			cnt[r]++;
		}else
		{
			swap(v,f(n+1,sum+a[n]));
		}
		vll fl=f(n+1,sum);
		for(ll i=0;i<v.sz;++i)
		{
