...
istream& operator>>(istream& is, mint& r) { is>>r.a;r.a+=MOD;r.a%=MOD; return is;} 
ostream& operator<<(ostream& os, const mint& r) { os<<r.a;return os;}
mint operator^(mint a, long long k) {
    mint r = 1; while (k) { if (k & 1) r *= a; a *= a; k >>= 1; } return r; }
...
ll modinv(ll a){
	ll m=MOD;
	function<pll(ll ,ll)> euclid=[&](ll a,ll b){
...
struct cm{
	ll x,y;
	cm(ll a,ll b):x(a),y(b){ };
	cm operator - (const cm r)const {
		return  cm(x-r.x,y-r.y);}

};


vll zarr(string text, string pattern) {
...
void sub(){
	...
	double pi=acos(-1);
	c=c*polar(1.0,pi*d/180);
	cout<<fixed<<std::setprecision(10)<<c.real()<<" "<<c.imag()<<endl;
