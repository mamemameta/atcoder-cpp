...
void sub()
{
	...
	for(auto [i,vv]:ru)
		ou(vv);
	ll ans=0;
	for(ll i=0;i<mp[w].sz;++i)
	{
		for(ll j=0;j<mp[w+1].sz;++j)
		{
			for(ll k=0;k<mp[w+2].sz;++k)
			{
				for(ll l=0;l<mp[w+3].sz;++l)
				{
					ll we=w*(i+1)+(w+1)*(j+1)+(w+2)*(k+1)+(w+3)*(l+1);
					ll sum=ru[w][i]+ru[w+1][j]+ru[w+2][k]+ru[w+3][l];
					if(we<=W)
						chmax(ans,sum);
				}
			}
