...
void sub() {
	...
	vec<mint> sa(B,0),sb(B,0);//バケットi内の総和
	vec<mint> sab(B,0);       //バケットi内のa*bの総和
	vec<mint> adda(B,0),addb(B,0);//バケットi内の加算された値の総和
	...
	while(Q--)
	{
		...
		}else
		{
			...
			if(lb==rb)
			{
				for(ll i=l;i<r;++i)
				{
					res += (a[i]+adda[lb])*(b[i]+addb[lb]);
				}
			}else
			{
				for(ll i=lr;i<D;++i)
				{
					res += (a[lb*D+i]+adda[lb])*(b[lb*D+i]+addb[lb]);
				}
				i(rr)
				{
					res+=(a[rb*D+i]+adda[rb])*(b[rb*D+i]+addb[rb]);
				}
				for(ll b=lb+1;b<rb;++b)
				{
					res += sab[b]+sa[b]*addb[b]+sb[b]*adda[b]+adda[b]*addb[b]*D;
				}
			}
			ff(res);
		}
