...
constexpr int K = 5;
int Mod[K] = {998244353, 1000000007, 1000000009, 1000000021, 1000000033};
int base[K];
...
using H = array<Hash, K>;//(ハッシュ値，)K個のハッシュ値

H gen(char c) {
	H res;
	for (int i = 0; i < K; i++) res[i].h1 = res[i].h2 = c, res[i].pw = base[i];
	return res;
}
...
struct segtree {
	...
	void set(int i, H x) { dat[i + n - 1] = x; }
	void build() {
		for (int k = n - 2; k >= 0; k--) dat[k] = opx(dat[2 * k + 1], dat[2 * k + 2]);
	}
	void update(int i, H x) {
		i += n - 1;
		dat[i] = x;
		while (i > 0) {
			i = (i - 1) / 2;  // parent
	...
	H query_sub(int a, int b, int k, int l, int r) {
		if (r <= a || b <= l) {  // 完全に外側の時
			return ex;
		} else if (a <= l && r <= b) {  // 完全に内側の時
			return dat[k];
		} else {  // 一部区間が被る時
			H vl = query_sub(a, b, k * 2 + 1, l, (l + r) / 2);
			H vr = query_sub(a, b, k * 2 + 2, (l + r) / 2, r);
			return opx(vl, vr);
		}
	}
	H query(int a, int b) { return query_sub(a, b, 0, 0, n); }
};
void sub() {
	...
	while (Q--) {
		...
		if (cmd == 1) {
			...
			auto h = sg.query(l, r);
			bool flag = true;
			for (int i = 0; i < K; i++) flag &= h[i].h1 == h[i].h2;
