...
void sub() {
	...
	vll ans;
	const ll D=600;
	for(ll l=0;l<Q;l+=D)//O(sqrt(Q))
	{
		ll r=min(Q,l+D);
		for(ll now=l;now<r;++now)//O(sqrtQ)
		{
			if(t[now]==1)
			{
				a[x[now]]^=y[now];
			...
			{
				ll res=sum[y[now]]^sum[x[now]-1];
				for(ll pre=l;pre<now;++pre)//O(sqrt(Q))600*600*600
				{
					if(t[pre]==1&&x[now]<=x[pre]&&x[pre]<=y[now])
					{
						res^=y[pre];
					}
				}
