...
void sub()
{
	...
	while(!q.em)
	{
		...
		if(x<X-1)
		{
			if(chmin(dis[y][x+1],c+cx[y][x]))
				q.push({dis[y][x+1],y,x+1});
		}
		if(x>0)
		{
			if(chmin(dis[y][x-1],c+cx[y][x-1]))
				q.push({dis[y][x-1],y,x-1});
		}
		if(y<Y-1)
		{
			if(chmin(dis[y+1][x],c+cy[y][x]))
				q.push({dis[y+1][x],y+1,x});
		}
		for(ll i=1;i<=y;++i)
		{
			if(chmin(dis[y-i][x],c+1+i))
				q.push({dis[y-i][x],y-i,x});
		}
	}
	ff(dis[Y-1][X-1]);
