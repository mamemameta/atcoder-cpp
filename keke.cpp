...
//#define mint modint<NOD>
#define NOD 1000000007
class mint { public: ll a; mint(ll x=0):a(x%NOD){} mint operator + (const mint rhs) const  { return mint(*this) += rhs; } mint operator - (const mint rhs) const  { return mint(*this) -= rhs; } mint operator * (const mint rhs) const  { return mint(*this) *= rhs; } mint operator / (const mint rhs) const  { return mint(*this) /= rhs; } mint &operator += (const mint rhs)  { a += rhs.a; if (a >= NOD) a -= NOD; return *this; } mint &operator -= (const mint rhs)  { if (a < rhs.a) a += NOD; a -= rhs.a; return *this; } mint &operator *= (const mint rhs)  { a = a * rhs.a % NOD; return *this; } mint &operator /= (mint rhs)  { ll exp = NOD - 2; while (exp) { if (exp % 2) *this *= rhs; rhs *= rhs; exp /= 2; } return *this; } bool operator > (const mint& rhs)const{ return (this->a>rhs.a); } bool operator < (const mint& rhs)const{ return (this->a<rhs.a); } bool operator >= (const mint& rhs)const{ return (this->a>=rhs.a); } bool operator <= (const mint& rhs)const{ return (this->a<=rhs.a); } };
/*
0->64
O(2*10^8)
9*10^18
