...
struct SegTree{
	...
	mint sub(ll l,ll r,ll nowl,ll now,ll nowr){
		if(nowr<l||r<nowl) re 0ll;
		if(l<=nowl&&nowr<=r)re v[now];
		mint vall=sub(l,r,nowl,(now*2)+1,(nowl+nowr)/2);
		mint valr=sub(l,r,(nowl+nowr)/2+1,(now*2)+2,nowr);
		re op(vall,valr);
	}
};
void sub()
{
	mod=998244353;
	ge(ll,n);
	SegTree cnt(2e5+5),sum(2e5+5);
	mint s=0;
	for(ll k=1;k<=n;++k)
	{
		mint ans=0;
		mint inv=mint(1)/mint(k)/mint(k);
		ge(ll,x);
		s+=mint(2)*cnt.ran(0,x)*mint(x)+mint(x);
		s+=sum.ran(x+1,2e5+3)*mint(2);
		ans=s*inv;
		ff(ans);
		cnt.update(x,1);
		sum.update(x,x);
