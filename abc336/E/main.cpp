...
void sub()
{
	...
	for(ll m=1;m<=9*17;++m)
	{
		vec dp(n+5,vec<vec<vll>>(2,vec<vll>(m+5,vll(m+5))));
		//dp[何文字目か][同じか][使った総和][mod]
		dp[0][1][0][0]=1;
		for(ll i=0;i<s.sz;++i)
		{
			for(ll j=0;j<=m;++j)
			{
				for(ll k=0;k<=m;++k)
				{
					for(ll c=0;c<=9;++c)
					{
						if(j+c<=m)
						{
							dp[i+1][0][j+c][((c+10*k))%m]+=dp[i][0][j][k];
						}
					}
					ll d=s[i]-'0';
					for(ll c=0;c<d;++c)
					{
						if(j+c<=m)
						{
							dp[i+1][0][j+c][((c+10*k))%m]+=dp[i][1][j][k];
						}
					}
					if(j+d<=m)
					{
						dp[i+1][1][j+d][((10*k+d))%m]+=dp[i][1][j][k];
					}
				}
			}
		}
