...
struct SG{
	...
	ll sub(ll a,ll b,ll k,ll l,ll r){
		if(r<=a||b<=l)return 0;
		if(a<=l&&r<=b)return v[k];
		ll vl=sub(a,b,k*2+1,l,(l+r)/2);
		ll vr=sub(a,b,k*2+2,(l+r)/2,r);
		return op(vl,vr);
	}
};
...
#define qll queue<ll>
ll di(ll n,ll s,ll g){
	vll d(n+1,M),vis(n+1);d[s]=0;
	priority_queue<pll,vec<pll>,greater<pll>>que;
	que.push(mkp(0,1));
	ll cst,now;
	while(!que.em){
		tie(cst,now)=que.top;que.pop();
		if(now==g)return d[g];
		if(vis[now])continue;
		vis[now]=1;
		/*
		for(auto [to,t,k]:eg[now]){
			ll c=(cst+k-1)/k*k+t;
			if(chmin(d[to],c))
				que.push({d[to],to});
		} */
...
pll dip(ll n,ll s,ll g){
	vec<vll>e(10);
	vll v(10);
	vll d(n+1,M);
	d[s]=0;
	priority_queue<vll,vec<vll>,greater<vll>>q;
	//fl("n",n,"s",s,"g",g); fl("v[s]",v[s]);
	vll el_={0,v[s],s};
	q.push(el_);
	//fl("seondn",n,"s",s,"g",g);
	wh(!q.em){
		ll dep=q.top[0],r=q.top[1],c=q.top[2];q.pop();
		//fl(c);
		if(c==g)return mkp(dep,r);
		fa(to,e[c]){
			if(chmineq(d[to],dep+1)){
				el_={dep+1,r+v[to],to};
				q.push(el_);
			}
		}
...
vec<vll> e(N); vll col(N);ll cnt[3]={};
ll dfs(ll c,ll w){
	col[c]=w;
	cnt[w]++;
	fa(to,e[c]){
