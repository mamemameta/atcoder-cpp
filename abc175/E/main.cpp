...
void sub() {
	ge(ll,r,c,k);
	vec<vll>v(r+5,vll(c+5));
	vec<vec<vll>>dp(r+5,vec<vll>(c+5,vll(5)));
	i(k)
	{
		ge(ll,y,x);
		ci(v[y][x]);
	...
	for(ll y=1;y<=r;++y)
	{
		for(ll x=1;x<=c;++x)
		{
			dp[y][x][0]=*max_element(all(dp[y-1][x]));
			i(4)
			{
				chmax(dp[y][x][i],dp[y][x-1][i]);
			}
			if(v[y][x])
			{
				for(ll i=3;i-1>=0;--i)
				{
					chmax(dp[y][x][i],dp[y][x][i-1]+v[y][x]);
				}
			}
		}
	}
	ff(*max_element(all(dp[r][c])));
}
