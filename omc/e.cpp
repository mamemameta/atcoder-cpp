...
void sub() {
	...
	for(ll a=1;a<=9;++a)
	{
		for(ll b=0;b<=9;++b)if(a!=b)
		{
			for(ll c=0;c<=9;++c)if(c!=a&&c!=b)
			{
				for(ll d=0;d<=9;++d)if(d!=a&&d!=b&&d!=c)
				{
					for(ll e=1;e<=9;++e)if(e!=a&&e!=b&&e!=c&&e!=d)
					{
						ll l=ten[0]*e+ten[1]*d+ten[2]*c+ten[3]*b+ten[4]*a;
						l*=a;
						ll r=ten[0]*e+ten[1]*e+ten[2]*e+ten[3]*e+ten[4]*e+ten[5]*e;
						if(l==r)
						{
							ff(l/a);
						}
