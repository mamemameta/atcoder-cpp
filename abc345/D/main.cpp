...
void sub() {
	...
	vll vl(n);
	iota(vl.be,vl.en,0);
	function<void(vll,ll,vec<vll>)>f=[&](vll vl,ll now,vec<vll>v)
	{
		ll x=0,y=0;
		while(v[y][x])
		{
		...
		if(y+a[j]<=h&&x+b[j]<=w)
		{
			vec<vll>t=v;
			ll ok=1;
			for(ll dy=0;dy<a[j];++dy)
			{
				for(ll dx=0;dx<b[j];++dx)
				{
					ok&=++t[y+dy][x+dx]==1;
				}
			}
		...
		if(a[j]!=b[j]&&y+b[j]<=h&&x+a[j]<=w)
		{
			vec<vll>t=v;
			ll ok=1;
			for(ll dy=0;dy<b[j];++dy)
			{
				for(ll dx=0;dx<a[j];++dx)
				{
					ok&=++t[y+dy][x+dx]==1;
				}
			}
	...
	do
	{
		vec<vll>v(h,vll(w));
		f(vl,0,v);
	}while(next_permutation(vl.be,vl.en));
	ff("No");
}
