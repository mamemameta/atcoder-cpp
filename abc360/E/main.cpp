...
void sub() {
	...
	ge(ll,N,K);
	mint ans=0;
	auto f=[&](auto f,ll k,mint p,mint q,mint r)->void
	{
		if(k==0)
		{
			ans+= p;
			ans+= mint(N+2)*mint(N-1)/mint(2)*q;
			re;
		}
		mint move=mint(2*(N-1))/mint(N*N);
		mint remain=mint(1)-move;
		mint come=mint(2)/mint(N*N);

		mint np=p*remain+mint(1-p)*come;
		mint nq=p*come+r*come+q*remain;
		f(f,k-1,np,nq,mint(1)-np-nq);
	};
	f(f,K,mint(1),mint(0),mint(0));
	ff(ans);
}
