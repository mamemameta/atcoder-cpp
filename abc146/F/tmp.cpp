...
template<class V, ll NV> struct LazySegTree { // [L,R)
	vector<V> dat, lazy;
       	LazySegTree() { 
		dat.resize(NV * 2, def);
	       	lazy.resize(NV * 2, ldef);
       	}
	void update(ll a, ll b, V v, ll k, ll l, ll r) {
		push(k, l, r);
	       	if (r <= a || b <= l) return;
		if (a <= l && r <= b)
	       	{ 
	...
	V get(ll a, ll b, ll k, ll l, ll r) {
		push(k, l, r);
	...
	void update(ll a, ll b, V v) 
	{ 
		update(a, b, v, 0, 0, NV);
       	}
	V get(ll a, ll b) { return get(a, b, 0, 0, NV); }
	// ---- Template ---------------------------------------------------------------------------------
	// 区間min代入,区間min
	const V def = infl, ldef = infl;
	V comp(V l, V r) { return min(l, r); }
	void setLazy(ll i, V v) { lazy[i] = min(lazy[i], v); }
...
ll N, M; string S;
LazySegTree<ll, 1 << 17> lst;
vector<ll> idx[101010];
void sub() {
	cin >> N >> M >> S;
	lst.update(N, N + 1, 0);
	rrep(i, N, 0) if(S[i] == '0') {
	...
	ll dp = lst.get(0, 1);
	while (cu < N) {
		ll nxt = *upper_bound(all(idx[dp - 1]), cu);
