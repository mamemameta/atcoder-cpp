...
void sub() {
	...
	segtree sg(n);
	segtree rsg(n);
	sg.set(0,segtree::X(0,0));
	rsg.set(0,segtree::X(0,0));
	fo(i,1,n-1)
	{
		sg.set(i,segtree::X(-1e17,c*i));
		rsg.set(i,segtree::X(-1e17,-c*i));
	}
	sg.build();
	rsg.build();
	ge(ll,q);
	xx(q)
	{
		ge(ll,t,p);
		--t;
		segtree::X xl=sg.query(0  ,t);
		ll l=xl.x+xl.c-c*t;

		segtree::X xr=rsg.query(t,n);
		ll r=xr.x+xr.c+c*t;

		ll mx=max(l,r)+p;
		sg.update(t,segtree::X(mx,c*t));
		rsg.update(t,segtree::X(mx,-c*t));
		/*
		i(n)
		{
			ff("i",i,sg.query(i,i+1).x);
		}
		ff(".........",q);
		///*/
	}
