...
void sub() {
	ge(ll,n);
	ge(dou,s,t);
	vec v(n,vec<vec<dou>>(2,vec<dou>(2)));
	i(n)
	{
		in(v[i][0]);
		in(v[i][1]);
	...
	vll p(n);
	iota(p.be,p.en,0ll);
	dou ans=2000*1000*10;
	auto dis=[&](dou x,dou y,dou xx,dou yy)->dou
	{
		dou dx=x-xx, dy =y-yy;
	...
	do
	{
		for(ll b=0;b<(1ll<<n);++b)
		{
			...
			for(ll i=0;i<n;++i)
			{
				ll j=p[i];
				ll o=(b>>p[i])%2;
				sum+=1./s*dis(x,y,v[j][o][0],v[j][o][1]);
				sum+=1./t*dis(v[j][0][0],v[j][0][1],v[j][1][0],v[j][1][1]);
				x=v[j][o^1][0];
				y=v[j][o^1][1];
			}
			chmin(ans,sum);
		}
	}while(next_permutation(p.be,p.en));
